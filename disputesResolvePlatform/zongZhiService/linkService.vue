<template>
	<view style="background-color: #1261d4;min-height: 100vh;">
		...
		<view class="scan-head">综治服务在线查询</view>
		<view v-if="scanshow" @click="handleIdentification()">
			...
			<view style="padding: 10rpx 50rpx 10rpx 50rpx;color: #FFFFFF;">
				<view>注意事项：</view>
					<view>1、本功能仅用于扫描综治信息系统（包括电脑端、微信小程序）内综治服务信息二维码使用；</view>
					<view>2、使用时请正确扫描综治服务信息相关记录二维码，非正确二维码系统会进行“二维码信息错误”提示；</view>
					<view>3、扫码成功后会自动跳转至您的申报列表、请您依据服务编号、时间等进行查找查看详情；</view>
					<view>4、如已经扫描关联过请勿重复扫描，系统会提示“已经关联过，请勿重复关联”；</view>
			</view>
		</view>
		<view v-if="mediateInfoShow">
			<view class="user-info">
				<view style="text-align: center;font-size: large;font-weight: 700;">综治服务信息</view>
				<!-- <view style="margin-left: 10%;">
					<u-icon name="file-text-fill" size="35" color="#1261d4" />服务编号:
				</view> -->
				<view style="text-align: center;">
					{{ mediateInfo.serviceNo }}
				</view>
				<view style="margin-left: 10%;">
					<u-icon name="file-text-fill" size="35" color="#1261d4" />申报人:{{ mediateInfo.linkman }} 
				</view>
			</view>
			<view class="refresh-user" @click="refreshUser()">
				<u-icon name="reload" color="#FFFFFF" size="35" style="margin-right: 2%;" />重新识别
			</view>
			<view style="margin: 10% 5% 0 8%;color: #FFFFFF;">
			...
			handleIdentification() {
				...
				wx.scanCode({
					success: (res) => {
						...
						if (JSON.parse(resResult).type == 2 ) {
							uni.showToast({
								title: '识别成功！！',
								icon: 'none'
							})
			...
			handleConfirm() {
				...
				this.$refs.mediateInfoData.validate(valid => {
					if (valid) {
						...
						ZongZhiService.linkzzfwOnWechat(postSta).then(res => {
							if (res.data.code === 0) {
								if (res.data.data === 0) {
									setTimeout(function() {
										uni.showToast({
