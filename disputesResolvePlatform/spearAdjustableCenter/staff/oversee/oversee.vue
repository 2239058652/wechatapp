<template>
	<view style="background-color: #ececec;min-height: 100vh;">
		...
		<view v-if="tabsIndex==0">
			...
			<view class="u-flex u-row-between" style="margin: 10px 40px 20px 40px;">
				<u-button type="primary" @click="handleCuiBan()" :loading="buttonLoading">
					纠纷调解催办
				</u-button>
				<u-button type="warning" @click="handleDuBan()" :loading="buttonLoading">
					纠纷调解督办
				</u-button>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			handleCuiBan(registId) {
				uni.navigateTo({
					url: '../comment/handleOversee?registId=' + this.registId + '&urgeType=' + 0
				})
			},
			handleDuBan(registId) {
				uni.navigateTo({
					url: '../comment/handleOversee?registId=' + this.registId + '&urgeType=' + 1
				})
			}
		}
	}
