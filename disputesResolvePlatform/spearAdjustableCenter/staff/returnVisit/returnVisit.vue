	...
	<view style="background-color: #ececec;min-height: 100vh;">
		...
		<view v-if="tabsIndex==0">
			...
			<view class="u-flex u-row-between" style="margin: 10px 40px 20px 40px;">
				<u-button type="primary" @click="handleJFHF()" :loading="buttonLoading">
					纠纷回访
				</u-button>
				<u-button type="success" @click="handleSFQR()" :loading="buttonLoading">
					司法确认
				</u-button>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			handleJFHF(registId) {
				uni.navigateTo({
					url: '../comment/handleAddJFHF?registId=' + this.registId
				})
			},
			handleSFQR(registId) {
				uni.navigateTo({
					url: '../comment/handleAddSFQR?registId=' + this.registId
				})
			}
		}
	}
