<template>
	<view>
		...
		<scroll-view scroll-y class="indexes" :scroll-with-animation="true" :enable-back-to-top="true" style="height: 999rpx;">
			<block v-for="(item,index) in list" :key="index">
				<view :class="'indexItem-' + item.name" :id="'indexes-' + item.name" :data-index="item.name">
					<view class="cu-list menu no-padding">
						<view class="cu-item">
							<u-checkbox @change="checkboxChange(item)" v-model="item.checked"
								:key="index" :name="item.name" size=45 />
							<view class="content">
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			if (options.coOrganizerIdList != '') {
				this.coOrganizerIdList = options.coOrganizerIdList.split(',')
			}
		...
		methods: {
			getList() {
				...
				OrgApi.getTargetOrgList(_self.listQuery).then(res => {
					this.$nextTick(function() {
						...
						_self.list.splice(_self.list.findIndex(i => i.id === _self.sponsorOrgId), 1)
					})
					uni.hideLoading()
				})
			},
			...
			checkboxChange(e) {
				...
				if(e.checked == undefined) {
					...
					_self.selectedList.splice(_self.selectedList.findIndex(i => i.id === e.id), 1)
			...
			selectedOrg(item) {
				uni.$emit('selectedOrgVice', this.selectedList)
				uni.navigateBack({
					delta: 1
				})
			}
