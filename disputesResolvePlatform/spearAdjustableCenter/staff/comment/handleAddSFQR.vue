<template>
	<view class="padding">
		...
		<view style="margin-top: 20vh;">
			<u-button type="primary" @click="handleAddSFQR()" :loading="buttonLoading">添加司法确认材料</u-button>
		</view>
	</view>
</template>
		...
		methods: {
			...
			handleAddSFQR() {
				...
				if (_this.tempCBDBfiles.length == 0) {
					...
					DisputesResolvePlatform.addConfirmRecord({
						files: oldFiles,
						registrationId: _this.registId
					}).then(res => {
						if (res.data.code === 0) {
							uni.showToast({
								title: '添加纠纷调解司法确认材料成功！',
								icon: 'none'
							})
							setTimeout(function() {
								uni.navigateBack({
