					...
					<view class="content" slot="body">
						...
						<view style="margin-top: 1vh;text-align: right;">
							<u-button v-if="tabsIndex == 0" type="primary" size="mini"
								:custom-style="customStyle"	@click="transferred(item)">纠纷调解</u-button>
							<u-button v-if="tabsIndex == 1" type="primary" size="mini" 
								:custom-style="customStyle" @click="handleReturn(item)">纠纷回访</u-button>
							<u-button type="success" size="mini" 
								:custom-style="customStyle" style="margin-left: 5%;" @click="handleView(item)">查看详情</u-button>
						</view>
					</view>
				</u-card>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			handleReturn(e) {
				uni.navigateTo({
					url: '../returnVisit/returnVisit?id=' + e.id
				})
			},
			transferred(e) {
				uni.navigateTo({
					url: './transferred?id=' + e.id
				})
			},
			tabsChange(index) {
				var _this = this
			...
			fetchList() {
				...
				DisputesResolvePlatform.getMediateList(_this.query).then(res => {
					if (res.data.code == 0) {
