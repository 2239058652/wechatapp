					...
					<view class="content" slot="body">
						...
						<view class="show-head-title">
							...
							<view class="titleRight">
								<u-tag :text="item.statusDesc" type="info" />
							</view>
						</view>
						<!-- 第二行 信息行-->
						...
						<view style="margin-top: 1vh;text-align: right;">
							<u-button v-if="tabsIndex == 0" type="primary" size="mini"
								:custom-style="customStyle" @click="handleShunt(item)">纠纷流转</u-button>
							<u-button v-if="tabsIndex == 1" type="warning" size="mini"
								:custom-style="customStyle" @click="handleOversee(item)">催办督办</u-button>
							<u-button type="success" style="margin-left: 5%;" size="mini"
								:custom-style="customStyle" @click="handleView(item)">查看详情</u-button>
						</view>
					</view>
				</u-card>
			</view>
	...
	export default {
		...
		methods: {
			...
			fetchList() {
				...
				DisputesResolvePlatform.getMediateRegistTransferList(_this.query).then(res => {
					if (res.data.code == 0) {
