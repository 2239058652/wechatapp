		...
		<view class="toppicker">
			<view style="width: 40%;">
				<u-input type="select" :select-open="reportTypeShow" input-align="center" v-model="typeText" :border="false"
				 placeholder="诉求类型" @click="reportTypeShow = true" />
				...
				<view class="content" @tap="handleView(item)">
					<view class="padding-tb-sm flex align-center">
						<view class="content margin-left-sm">
							<view>
								<text class="text-black" style="font-weight: bold;font-size: 34rpx;">诉求类型：{{item.appealTypeDesc}}</text>
							</view>
							<view class="text-sm">
...
<script>
	...
	export default {
		...
		methods: {
			...
			reportTypeChange(e) {
				if (this.reportTypeList[e[0]].label == '诉求类型') {
					this.typeText = '诉求类型'
					this.query.appealType = ''
				} else {
					this.typeText = this.reportTypeList[e[0]].label
					this.query.appealType = this.reportTypeList[e[0]].value
				}
				this.fetchList()
			},
			fetchList() {
				...
				MassAppealApi.getMassAppealList(_this.query).then(res => {
					if (res.data.code == 0) {
