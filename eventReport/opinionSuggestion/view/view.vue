	...
	export default {
		...
		methods: {
			...
			fetchDetail() {
				...
				OpinionSuggestionApi.getOpinionSuggestionDetail(this.reportId).then(res => {
					if (res.data.code == 0) {
			...
			handelAnswer() {
				if (this.anwserDataTemp.content == '') {
					...
					OpinionSuggestionApi.appealReportAnswer(this.anwserDataTemp).then(res => {
						if (res.data.code == 0) {
							uni.showToast({
								title: '回复成功。',
								icon: 'none'
