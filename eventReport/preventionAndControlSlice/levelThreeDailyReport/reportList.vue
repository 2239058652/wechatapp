								...
								<view class="content">
									<view class="padding-tb-sm flex align-center">
										<view class="content margin-left-sm">
											<view>
												<text class="text-black" style="font-weight: bold;font-size: 30rpx;text-align: center;width: 100%;">
													{{item.reportTime}} 片长日报
												</text>
											</view>
											<view>
...
<script>
	...
	export default {
		...
		methods: {
			handleView(e) {
				uni.navigateTo({
					url: '/eventReport/preventionAndControlSlice/levelFourDailyReport/view?id=' + e.id
				})
			},
			fetchEventList() {
				var _this = this
				...
				PreventionAndControlSlice.getLevelThreeAreaLeaderReportOwnerList(_this.query).then(res => {
					if (res.data.code == 0) {
