...
<script>
	...
	export default {
		...
		methods: {
			handleView(e) {
				uni.navigateTo({
					url: '/eventReport/preventionAndControlSlice/householderDailyReport/view?id=' + e.id
				})
			},
			fetchEventList() {
				var _this = this
				...
				PreventionAndControlSlice.getHouseholderOwnerReport(_this.query).then(res => {
					if (res.data.code == 0) {
