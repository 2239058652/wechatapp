										...
										<view class="content margin-left-sm">
											<view>
												<text class="text-black" style="font-weight: bold;font-size: 30rpx;">
													填报日期：{{item.reportTime}}
												</text>
											</view>
											<view>
												<text class="text-black">
													非本村可疑外来人员：{{item.markedPeopleListVoLength}} 人
												</text>
											</view>
											<view>
												<text class="text-black">
													深夜外出村民：{{item.levaePeopleListVoLength}} 人
												</text>
											</view>
											<view>
												<text class="text-black">
													境外务工人员：{{item.overseasPeopleListVoLength}} 人
												</text>
											</view>
											<view>
												<text class="text-black">
													有境外务工史人员：{{item.historyOverseasPeopleListVoLength}} 人
												</text>
											</view>
											<view>
												<text class="text-black">
													{{item.orgName}}——{{item.userName}}于{{item.createdDate}}填报
												</text>
											</view>
										</view>
									</view>
...
<script>
	...
	export default {
		...
		methods: {
			handleView(e) {
				uni.navigateTo({
					url: '/eventReport/preventionAndControlSlice/villagerDynamicMonitoring/view?id=' + e.id
				})
			},
			fetchEventList() {
				var _this = this
				...
				PreventionAndControlSlice.getBorderAreaMonitorReportList(_this.query).then(res => {
					if (res.data.code == 0) {
						_this.$nextTick(function() {
							...
							_this.showEventList.forEach(element => {
								element.markedPeopleListVoLength = element.markedPeopleList.length
								element.levaePeopleListVoLength = element.levaePeopleList.length
								element.overseasPeopleListVoLength = element.overseasPeopleList.length
								element.historyOverseasPeopleListVoLength = element.historyOverseasPeopleList.length
							})
							_this.showLen = len
