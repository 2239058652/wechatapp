...
<script>
	...
	export default {
		...
		methods: {
			handleView(e) {
				uni.navigateTo({
					url: '/eventReport/preventionAndControlSlice/householdIncidentReport/view?id=' + e.id
				})
			},
			fetchEventList() {
				var _this = this
				...
				PreventionAndControlSlice.getBorderAreaEventReportList(_this.query).then(res => {
					if (res.data.code == 0) {
						_this.$nextTick(function() {
							...
							_this.showEventList.forEach(element => {
								element.peopleLength = element.peopleList.length
							})
							_this.showLen = len
