		...
		<view v-if="sectionCurrent==0" class="form">
			<u-form :model="dataTemp" ref="uForm" label-width="140" :error-type="errorType">
				...
				<u-form-item label="涉案人数:">
					{{dataTemp.peopleLength}} 人
				</u-form-item>
				<view class="title-index">涉案人数: {{dataTemp.peopleLength}} 人</view>
				<view class="item" v-for="(item, index) in dataTemp.peopleList" :key="item.id">
	...
	export default {
		...
		methods: {
			...
			fetchDetail() {
				...
				PreventionAndControlSlice.viewBorderAreaEventReportDetail(_this.reportId).then(res => {
					if (res.data.code == 0) {
						...
						_this.dataTemp.peopleLength = _this.dataTemp.peopleList.length
