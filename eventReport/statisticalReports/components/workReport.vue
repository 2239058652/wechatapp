...
<script>
	...
	export default {
	    ...
	    data() {
	        return {
	            ...
	            headers: [
					...
					{ label: '报送总数(条)', key: 'total', width: '210' },
					{ label: '网格巡查(条)', key: 'type1', width: '210' },
					{ label: '重点人群管理(条)', key: 'type2', width: '250' },
					{ label: '法制宣传(条)', key: 'type3', width: '210' },
					{ label: '其他日常工作情况(条)', key: 'type4', width: '200' }
				],
	            contents: [],
				computedCol: ['total', 'type1', 'type2', 'type3', 'type4' ],
				sortCol: [
					{ key: 'total', isNumber: true },
					{ key: 'type1', isNumber: true },
					{ key: 'type2', isNumber: true },
					{ key: 'type3', isNumber: true },
					{ key: 'type4', isNumber: true }
				],
				starTimeShow: false,
				endTimeShow: false
	        }
		...
		methods: {
			...
			fetchList() {
				this.loading = true
				StatisticalReports.getWorkReportStatistics(this.query).then(res => {
					if (res.data.code == 0) {
