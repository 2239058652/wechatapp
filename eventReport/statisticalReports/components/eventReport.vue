...
<script>
	...
	export default {
	    ...
	    data() {
	        return {
	            ...
	            headers: [
					...
					{ label: '报送总数(件)', key: 'total', width: '210' },
					{ label: '办理中(件)', key: 'type3', width: '180' },
					{ label: '已办结(件)', key: 'type1', width: '180' },
				],
	            contents: [],
				computedCol: ['total', 'type2', 'type3' ],
				sortCol: [
		...
		methods: {
			...
			fetchList() {
				this.loading = true
				StatisticalReports.getTroubleReportStatistics(this.query).then(res => {
					if (res.data.code == 0) {
