<template>
	<view class="content">
		<view class="cu-list menu sm-border">
			<view v-for="(item,index) in showList" :key="item.eventReport.id" class="cu-item">
				<view class="content" @tap="handleDetail(item)">
					<view class="padding-tb-sm flex align-center">
						<view class="content margin-left-sm">
							<view>
								<text class="text-black" style="font-weight: bold;font-size: 34rpx;">{{item.eventReport.content | contentFilter}}</text>
							</view>
							<view class="text-sm">
	...
	export default {
		...
		methods: {
			...
			handleDetail(e) {
				console.log(e)
				uni.navigateTo({
					url: '../detail/detail?id=' + e.eventReport.id + '&nodeid=' + e.node.id
				})
			},
			fetchList() {
				var _this = this
				...
				EventWorkOutApi.getEventLeaderApproveList(_this.query).then(res => {
