<template>
	<view style="background-color: #ececec;min-height: 100vh;">
		...
		<view v-for="(item,index) in showList" :key="item.id" class="cu-item">
			<!-- <u-card :show-head="false" style="margin-bottom: 2vh;" @tap="handleView(item)"> -->
				...
				<view class="content" slot="body">
					...
					<view class="show-body">
						<view class="bodyTop">
							...
							平安豆值: <u-tag :text="item.value" type="error" size="mini" />
						</view>
						<view class="bodyTop">
		...
		<u-popup v-model="showDayIntegralList" mode="center" width="600rpx" height="1100upx">
			<view style="padding:10px;display: flex;width: 100%;height: 100rpx;text-align: center;">
				<view style="font-size: larger;font-weight: bold;width: 70%;text-align: left;">
					<view style="color: #0081FF;">获取平安豆：{{ clickIntegral }} 个</view>
					<view style="color: #f29100;">使用平安豆：0 个</view>
				</view>
				<view style="width: 30%;text-align: right;line-height: 100rpx;">{{ clickDay }}</view>
			</view>
			<view class="padding">
				<u-time-line>
					<u-time-line-item v-for="(item,index) in dayIntegralList" :key="index">
						<!-- 此处没有自定义左边的内容，会默认显示一个点 -->
						<template v-slot:node>
							<view class="u-node" style="background: #FFFFFF;">
								<u-icon name="integral" color="#0081FF" :size="35"></u-icon>
							</view>
						</template>
						<template v-slot:content>
							<view style="height: 100rpx;width: 100%;display: flex;">
								<view style="width: 75%;line-height: 50rpx;">
									<view>{{item.createdDate}}</view>
									<view style="font-weight: bold;color: #000000;">{{item.reason}}</view>
								</view>
								<view style="width: 25%;text-align: right;line-height: 100rpx;color: #0081FF;">
									<view><text style="font-size: larger;font-weight: bold;">+{{item.value}}</text> 个</view>
								</view>
							</view>
						</template>
					</u-time-line-item>
