			...
			<view style="padding: 10rpx 50rpx 10rpx 50rpx;color: #FFFFFF;">
				<view>注意事项：</view>
					<view style="text-indent: 2rem">
						1、本功能仅用于景洪市委政法委扫描综治信息系统内商家中心兑换信息二维码使用；
					</view>
					<view style="text-indent: 2rem">
						2、使用时请正确扫描综治信息系统内商家中心兑换信息二维码，非正确二维码系统会进行“二维码信息错误”提示；
					</view>
					<view style="text-indent: 2rem">
						3、扫码成功后会自动扣减用户平安豆至您的账户内，您的平安豆会自动累加，请至您的兑换记录进行查看；
					</view>
					<view style="text-indent: 2rem">
						4、如扫描失败，系统会提示相关错误原因，如有疑问请与景洪市委政法委进行联系；
					</view>
			</view>
		</view>
		<view v-if="userInfoShow">
			...
			<view style="margin: 0 5% 0 8%;">
				<u-form :model="dataTempDH" ref="uFormDCJL" :error-type="errorType">
					...
					<u-form-item :border-bottom="false" label="兑换描述:" prop="desc" style="color: #FFFFFF;" label-width="140">
						<view style="background-color: #FFFFFF;width: 100%;">
							<u-input v-model="dataTempDH.desc" :border="true" type="textarea"
								height="150" style="background-color: #FFFFFF;"/>
						</view>
					</u-form-item>
				</u-form>
			</view>
	...
	export default {
		...
		methods: {
			handleIdentification() {
				...
				wx.scanCode({
					success: (res) => {
						if (JSON.parse(res.result).type == '2' ) {
							uni.showToast({
								title: '识别成功！！',
								icon: 'none'
							})
