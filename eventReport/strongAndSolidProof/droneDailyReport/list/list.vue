								...
								<view class="content">
									<view class="padding-tb-sm flex align-center">
										<view class="content margin-left-sm">
											...
											<view>
												<text class="text-black">
													巡逻时长：{{item.patrolDuration}} 分钟
												</text>
											</view>
											<view>
												<text class="text-black">
													巡逻情况：{{item.patrolDetail}}
												</text>
											</view>
										</view>
									</view>
...
<script>
	...
	export default {
		...
		methods: {
			handleView(e) {
				uni.navigateTo({
					url: '/eventReport/strongAndSolidProof/droneDailyReport/view/view?id=' + e.id
				})
			},
			fetchEventList() {
				var _this = this
				...
				StrongAndSolidProof.getDronePatrolLogList(_this.query).then(res => {
					if (res.data.code == 0) {
