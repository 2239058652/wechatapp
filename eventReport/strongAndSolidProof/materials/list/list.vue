...
<script>
	...
	export default {
		...
		methods: {
			handleView(e) {
				uni.navigateTo({
					url: '/eventReport/strongAndSolidProof/materials/view/view?id=' + e.id
				})
			},
			fetchEventList() {
				var _this = this
				...
				StrongAndSolidProof.getLogisticsMaterialReportList(_this.query).then(res => {
					if (res.data.code == 0) {
