<template>
	<view class="u-wrap">
		<view class="u-search-box">
			<view class="u-search-inner" @click="handleToSearch()">
				<u-icon name="search" color="#909399" :size="30" />
			</view>
		</view>
...
<script>
	...
	export default {
		...
		computed: {},
		onLoad(option) {
			console.log('11111111111111111', option)
		...
		methods: {
			fechSituationTree() {
				this.situationTree = []
				SituationApi.getSceneClsTree().then(res => {
			...
			handleToSearch() {
				uni.navigateTo({
					url: '/eventReport/eventhandle/detail/scenarioDetailList?eventId=' 
					+ this.eventId  + '&nodeId=' + this.nodeId
				});
			},
			// 前往事件类别细项
			handleToScenario(e) {
				console.log('AAAAAAAAAAAAAAAAAAAAAAAAA', this.eventId)
				uni.navigateTo({
					url: '/eventReport/eventhandle/detail/fineItemList?scenarioId=' 
