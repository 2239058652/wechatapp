<template>
	<view class="content">
		<view class="cu-list menu sm-border">
			<view v-for="(item,index) in showList" :key="item.id" class="cu-item" @tap="handleDetail(item)">
				<view class="content">
				...
				<!-- <view class=" action" style="margin-top: -40upx;">
					...
					<view class="flex justify-end padding-top-sm">
						<text class="text-bold">{{item.qianYueJE}}</text>
					</view>
				</view> -->
			</view>
...
<script>
	import EventApi from '../../../api/eventreportgrid.js'
	...
	export default {
		...
		data() {
			return {
				...
				query: {
					...
					id: '',
					keHuMC: '',
					heTongMC: '', //this.$moment().format('YYYY-MM-DD'),
					qianDingRXM: '', //this.$moment().format('YYYY-MM-DD'),
					yuJiQYRQStart: '',
					yuJiQYRQEnd: ''
				},
				value: '',
		...
		methods: {
			...
			fetchList() {
				...
				EventHandleApi.getEventSplitList(_this.query).then(res => {
